        ...
        if (event.target.classList.contains('remove-item')){
            ...
            addItem.forEach(item => {
                if (item.FoodID === id){ 
                    ...
                    if ( window.location != 'http://127.0.0.1:5501/tit-canteen-site-master/admin-side.html'){
        ...
        // When lower Quantity is clicked
            ...
            addItem.forEach(item => {                
                if (item.FoodID === id && item.Quantity >= 1){
                    item.Quantity -= 1;
                    if (item.Quantity === 0){
                        if ( window.location != 'https://tit-canteen.netlify.app/user-orders.html'){
...
function ClientDataFlow(addToCartBtn){
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            ...
            checkOutBtn.addEventListener('click', ()=>{
                ...
                if (addItem.length != 0){
                    ...
                    window.setTimeout(function(){
                        window.location.replace('https://tit-canteen.netlify.app/user-orders.html')
                    }, 2600)
                } else {
                    Swal.fire({
                        icon: 'error',
            ...
            // Shows Orders
            if (window.location.href === 'https://tit-canteen.netlify.app/user-orders.html'){
